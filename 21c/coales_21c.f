...
c	Conservation of net baryon.
	...
	enddo
	if(nap.gt.1 .and. nat.gt.1)netba=nap+nat-netba
	if(nap.eq.1 .and. nat.eq.1)netba=nzp+nzt-netba
...
c     hadron and baryon when first run.
...
200	  continue
	  ...
	  idp(ii)=idp(j) ! j is the number of the particle having maximal energy
	  rmp(ii)=rmp(j)
	  do jj=1,4
	    pp(jj,ii)=pp(jj,j)
	    rp(jj,ii)=rp(jj,j)
...
c	No q, so there cannot compose meson, throw away nqb.
