...
c	Conservation of net baryon.
	...
	kf=kbh(i1,2)
	kfab=iabs(kf)
cc	sumchh=sumchh+pychge(kf)
	if(kf.gt.0.and.(kf.gt.1000 .and. kf.lt.10000))netba=netba+1
	if(kf.lt.0.and.(kfab.gt.1000 .and. kfab.lt.10000))netba=netba-1
	enddo
c070905	
	inetba=0
	if(kfaa.eq.2212 .or. kfaa.eq.2112)inetba=inetba+1
	if(kfaa.eq.-2212)inetba=inetba-1   ! in case of -2112 do not call pythia
	if(kfbb.eq.2212 .or. kfbb.eq.2112)inetba=inetba+1
	if(kfbb.eq.-2212)inetba=inetba-1   ! in case of -2112 do not call pythia
	netba=inetba-netba    
c070905
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	subroutine updth(n2,n1,i)
c     Move 'idpth',etc. i steps forward since n1+1
c	n2 : the last order # of parton list
      IMPLICIT DOUBLE PRECISION(A-H, O-Z)
      IMPLICIT INTEGER(I-N)
      INTEGER PYK,PYCHGE,PYCOMP
	...
	jj=n2+i
        ...
        rmpth(jj)=rmpth(j)
        do j1=1,4
        ppth(j1,jj)=ppth(j1,j)
        rpth(j1,jj)=rpth(j1,j)
        enddo
	endif
	if(n2.gt.n1+1)then
	do j=n2,n1+1,-1
...
c	ich : total charge of the partons thrown away
	...
	common/throqb/iprlth,nonth,rpth(4,mplis),ppth(4,mplis),
     c   idpth(mplis),rmpth(mplis)   ! 070905
	dimension pc(4),rc(3),rcp(3),iar(3)
...
503     continue
	...
	enddo
	enddo
	ithroq=ithroq+n1-nqb   ! 070905
	iprlth=iprlth+n1-nqb   ! 070905
	n1=0

c	Reinitiate 'parlist'
        do j=1,mplis
        idp(j)=0
        rmp(j)=0.
        taup(j)=0.
