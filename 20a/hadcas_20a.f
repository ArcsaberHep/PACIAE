...
c280910	param(1)=para1_2   ! 250204
...
c280910	call sysini_h   ! it has been called in paciae.f
...
c060404	ddt=PARAM(8)
	ddt=adj1(11)   ! 060404
c	write(9,*)'ddt,adj1(11)=',ddt,adj1(11)
c        rao=PARAM(10)
        kfmax=KFMAXT
        do i=1,100
        kfaco(i)=KFACOT(i)
...
500	continue
	if(tc(nctl).le.1.e-7) nctl=nctl-1
        ...
        tw(i)=0.
        enddo
c        write(9,*)'be get off ctlcre nctl=',nctl
        return
        end
...
c	neve: total number of runs
     ...
     c  ,rnt,rnp,ecsspn,ecsspm
        ...
        integer winel
c	write(9,*)'in scat sig,rcsit,nctl=',sig,rcsit,nctl   ! sa
c	call prt_sa1_h(nsa) ! sa
c	do iop=1,nctl   ! sa
c	write(9,*)iop,lc(iop,1),lc(iop,2),tc(iop)   ! sa
c	enddo
...
c       write(9,*)'after usual and call decay ijk,iii=',ijk,iii   ! sa
c       ich2=0.   !!
c       do i1=1,nsa   !!
c       kf=ksa(i1,2)   !!
c       ich2=ich2+pychge(kf)   !!
c       enddo   !!
c       if(ich1.ne.ich2)then   !!
c       write(9,*)'after usual'   !!
c       write(9,*)'ijk,iii,c1,c2=',ijk,iii,ich1,ich2   ! sa
c       endif
...
c       momentum modular of scattered particle in cms   100111
...
c	if(d.lt.1.e-10)return
	pt=0.5   ! 100111
	a=min(10.3,1./(1.12*pt)/(1.12*pt))   ! 100111 become active
c	d6=d**6
c	b=d6*a/(1.+d6)
...
c	reaction 
	...
	write(9,*)'nothing is found,kl,kl1=',kl,kl1
13	do m=3,5
	lc(icp,m)=0
	enddo	
	tw(icp)=0.
...
c     	if(lc(icp,5).ge.593)write(9,*)'annih. occur'
	endif
	endif
999	return
	end	
...
c        if((iiii.eq.12 .and. iii.eq.57) .or.
c     c  (iiii.eq.20 .and. iii.eq.94))then   !!!
c        write(9,*)'in updpli l,l1,kf1,kf2=',l,l1,kf1,kf2   !!!
c        endif   !!!
c010706
c	if(ik2.eq.22)then
c        write(9,*)'in updpli kf1,kf2,ik1,ik2,kfmax=',kf1,kf2,ik1,ik2,
c     c	 kfmax ! sa
c	write(9,*)'kfaco=',(kfaco(i1),i1=1,53)   ! sa 
c	endif
c010706
 	ll=l
	ll1=l1
...
c	if(ik2.eq.22)write(9,*)'numb=',(numb(i1),i1=1,53)   ! sa 010706
	do 600 j=1,kfmax
...
cc	if(nnn.eq.1)goto 300
	...
	numb(m)=numb(m)+1
	enddo
c	if(ik2.eq.22)write(9,*)'j,numb(j)=',j,numb(j)   ! sa 010706 
	goto 2000
cc300	goto 200
...
c        if((iiii.eq.12 .and. iii.eq.57) .or.
c     c  (iiii.eq.20 .and. iii.eq.94))then   !!!
c        write(9,*)'kf,ll,ll1=',kf,ll,ll1   !!!
c        write(9,*)'i,j,jj,n=',i,j,jj,n   !!!
c	write(9,*)'numb=',(numb(i1),i1=1,32)   !!!
c        call prt(n)   !!!
c        do i1=1,nctl   !!!
c        lc1=lc(i1,1)   !!!
c        lc2=lc(i1,2)   !!!
c        write(9,*)'i1,lc1,lc2,k1,k2=',i1,lc1,lc2,k(lc1,2),k(lc2,2)   !!!
c        enddo   !!!
c	endif   !!!
	if(i.eq.2)goto 500
	ll2=ll
...
c        endif
c        enddo
c        write(9,*)'in updpli n,n22=',n,n22   ! sa
c010706
...
c	calculate the collision time & fill up lc(i,1-2),tc(i). 
	...
	pj(4)=pel1
	do i=1,3
c	write(9,*)'p,psa=',p(l,i)
c	write(9,*)'p1,psa1=',p(l1,i)
	pi(i)=p(l,i)
	pj(i)=p(l1,i)
	b(i)=(pi(i)+pj(i))/(pi(4)+pj(4))
	enddo
...
c311002
	...
	ri(i)=v(l,i)
	rj(i)=v(l1,i)
c	write(9,*)'v,vsa=',v(l,i)
c	write(9,*)'v1,vsa1=',v(l1,i)
	enddo
cc	ri(4)=time
cc	rj(4)=time
...
c	deal with decay of decaying particles one by one
	jj=iwide(i)
	kf=ksa(jj,2)
c	if(iiii.eq.2 .and. iii.eq.147)write(9,*)'in decay kf=',kf   !!
	do i1=1,5
	pp(i1)=psa(jj,i1)
	enddo
	do i1=1,3
	...
	rr(4)=time
	ee=pp(4)
c	write(9,*)'jj,kf,psa4,ee=',jj,kf,psa(jj,4),ee   !!
