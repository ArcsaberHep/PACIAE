...
c	 distribution
	...
	enddo
c       p+A
        elseif(ipden.eq.0 .and. itden.eq.1)then   !!
        do i=1,3
        c17(1,i)=0.
        enddo
c	distribute target nucleons
	...
	enddo
        elseif(ipden.eq.0 .and. itden.eq.0)then   !!
        do i=1,3
        c17(1,i)=0.
        c17(2,i)=0.
        enddo        
...
c      '1 -> nzp' are projectile protons, 'nzp+1 -> nap' are projectile 
...
c	call pydbrb(1,n,0.0,0.0,bst(1),bst(2),bst(3))
c	Lorentz contract
	bzp3=0.
	bzp4=0.
	bzt3=0.
...
cs	write(9,*)'af. scat, iii,nsa,naf,nctl=',iii,nsa,naf,nctl   ! sa
        if(ijk.eq.1)return   
...
c        rnp=(3.*anap/(4.*3.1415926*rou0))**(0.33333)
	rp00=1.05
	rt00=1.05
	if(nap.gt.16)rp00=1.16*(1-1.16*anap**(-0.666666))
	if(nat.gt.16)rt00=1.16*(1-1.16*anat**(-0.666666))
...
c       in the program the x-sections are given in a unit of fm.
        csnn=PARAM(1)*0.1
        cspin=PARAM(2)*0.1
        cskn=PARAM(3)*0.1
        cspipi=PARAM(4)*0.1
...
c	 respectively
c       (b-x,y,z) is inside or not not inside the sphere of projectile
...
c       write(9,*)'in hadron cascade iii,tcp,time=',iii,tcp,time   ! 111010
...
c	istop=1 means all particles have get out of considered volume
	...
	m3=numb(3)
        m4=numb(4)
	m7=numb(7)
	if((l.le.m2 .and. l1.le.m2) .and. ss.ge.parp21)then   ! if 1 011210
...
c       calculate the 'orentation' of the vector pi
	...
	if(kfa.eq.2212.and.kfb.eq.2212)then   ! if 2
c200601
        kpp=kpp+1
...
c	write(9,*)'after calling pyevnt'   !s
	...
	elseif(kfa.eq.2212.and.kfb.eq.2112)then
c200601
        knp=knp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
c200601
	...
	elseif(kfb.eq.2212.and.kfa.eq.2112)then
c200601
        knp=knp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
c200601
	...
	elseif(kfa.eq.2112.and.kfb.eq.2112)then
c200601
        knn=knn+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
c200601
	call pyinit('cms','n0','n0',ss)   
        call pyevnt
c011210
c       for p+pbar or pbar+p
        elseif(kfa.eq.2212.and.kfb.eq.-2212)then
        kpp=kpp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        call pyinit('cms','p','pbar',ss)
        else
        call pyinit('cms','pbar','p',ss)
        endif
        call pyevnt

        elseif(kfb.eq.2212.and.kfa.eq.-2212)then
        kpp=kpp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        ...
        call pyevnt
c       for p+nbar or nbar+p
        elseif(kfa.eq.2212.and.kfb.eq.-2112)then
        knp=knp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        call pyinit('cms','p','nbar',ss)
        else
        call pyinit('cms','nbar','p',ss)
        endif
        call pyevnt
        ...
        call pyevnt
c       for n+nbar or nbar+n
        elseif(kfa.eq.2112.and.kfb.eq.-2112)then
        knn=knn+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        call pyinit('cms','n','nbar',ss)
        else
        call pyinit('cms','nbar','n',ss)
        endif
        call pyevnt

        elseif(kfb.eq.2112.and.kfa.eq.-2112)then
        knn=knn+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        ...
        call pyevnt
c       for n+pbar or pbar+n
        elseif(kfa.eq.2112.and.kfb.eq.-2212)then
        knp=knp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
        call pyinit('cms','n','pbar',ss)
        else
        call pyinit('cms','pbar','n',ss)
        endif
        call pyevnt

        elseif(kfb.eq.2112.and.kfa.eq.-2212)then
        knp=knp+1
        if(pti.le.1.e-4)kpar=kpar+1
        if(ptj.le.1.e-4)kpar=kpar+1
        if(cctai.gt.0.)then
	...
	elseif(kfa.eq.2212.and.kfb.eq.211)then
        if(pti.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','p','pi+',ss)
	else
        call pyinit('cms','pi+','p',ss)
	endif
        call pyevnt

	elseif(kfb.eq.2212.and.kfa.eq.211)then
        if(ptj.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','pi+','p',ss)
	else
	...
	elseif(kfa.eq.2212.and.kfb.eq.111)then
        if(pti.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','p','pi0',ss)
	else
        call pyinit('cms','pi0','p',ss)
	endif
        call pyevnt

	elseif(kfb.eq.2212.and.kfa.eq.111)then
        if(ptj.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','pi0','p',ss)
	else
	...
	elseif(kfa.eq.2112.and.kfb.eq.211)then
        if(pti.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','n0','pi+',ss)
	else
        call pyinit('cms','pi+','n0',ss)
	endif
        call pyevnt

	elseif(kfb.eq.2112.and.kfa.eq.211)then
        if(ptj.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','pi+','n0',ss)
	else
	...
	elseif(kfa.eq.2112.and.kfb.eq.111)then
        if(pti.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','n0','pi0',ss)
	else
        call pyinit('cms','pi0','n0',ss)
	endif
        call pyevnt

	elseif(kfb.eq.2112.and.kfa.eq.111)then
        if(ptj.le.1.e-4)kpar=kpar+1
	if(cctai.gt.0.)then
	call pyinit('cms','pi0','n0',ss)
	else
...
c080104
c300705
	if(adj140.eq.1)goto 9002   ! run ended before parton cascade
c	'pyjets' to 'parlist'
        iprl=n
        do i=1,n
        idp(i)=k(i,2)
...
c121110
9002    if(adj140.ne.4)then   ! 200905
c270905
        naf0=naf
c       'pyjts' to 'saf'
        if(n.ge.1)then   ! 3
...
c080104
cs      write(9,*)'af. updpip nsa,nctl=',nsa,nctl   ! sa
cs      call prt_sa2(nsa)   ! sa
cs      call prt_saf(naf)
cs      do i1=1,nctl
cs      write(9,*)'i1,lci,lcj,t=',i1,lc(i1,1),lc(i1,2),tc(i1)   ! sa
cs      enddo
c080104
        l=lc(icp,1)
        l1=lc(icp,2)
...
cs      write(9,*)'af. updtlp iii,nsa,nctl=',iii,nsa,nctl   ! sa
cs      call prt_sa2(nsa)
cs      call prt_saf(naf)
cs      do i=1,nctl
cs      write(9,*)'i,lci,lcj,t=',i,lc(i,1),lc(i,2),tc(i)
cs      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine remo
c       move hadrons from 'pyjets' to 'sbh' 
      PARAMETER (KSZJ=40000)
      IMPLICIT DOUBLE PRECISION(A-H, O-Z)
      IMPLICIT INTEGER(I-N)
      INTEGER PYK,PYCHGE,PYCOMP   
...
c	create initial collision list  
	...
	nzpt=nzpab+nztab
	napt=nap+nat
	do 10 l=1,nzpab   ! projectile proton
...
c	endif
	...
	if(dmin.gt.ecsnn)return
c	distance between the two particles should be smaller than ecsnn
	do ik=1,3
...
c	 and needs to put in particle list
c	iii : current collision number
	...
	common/sa14/ipyth(2000),idec(2000),iwide(2000)
        common/papr/t0,sig,dep,ddt,edipi,epin,ecsnn,ekn,ecspsn,ecspsm
     c  ,rnt,rnp,rao,rou0,vneu,vneum,ecsspn,ecsspm
c	ipyth: stord the order number (in the particle list) of 'pythia' 
c	 particles  
        dimension lc(nsize,5),tc(nsize),tw(nsize)
	dimension peo(4)
	do m=1,2000
...
c	m7=numb(7)  
	do j11=1,nup
	j1=ipyth(j11)
	if(j1.le.m2)goto 301   ! 221110
c	consider only the reinteraction among nucleons 
	goto 300
301	mm=m2   
	do i=1,mm   
c	consider only the reinteraction among nucleons 
